...
type Control[S hypp.State] interface {
	...
	UpdateFromEvent(state S, event hypp.Event) S
...
func New[S hypp.State](
	...
	var tale *Tale[S]
	var dispatch func(dispatchable hypp.Dispatchable, props hypp.Payload)
	dispatch = func(dispatchable hypp.Dispatchable, props hypp.Payload) {
		...
		case hypp.Action[S]:
			dispatch(v(tale.state, props), nil)
